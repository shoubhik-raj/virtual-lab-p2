...
const SimulationBuilder: React.FC = () => {
  ...
  const [simulationName, setSimulationName] = useState<string>("Fire Alarm Simulation");
  ...
  const handlePublishSimulation = async () => {
    ...
    try {
      ...
      const simulationData = {
        name: simulationName,
        subject: "Physics", // Default subject, could make this configurable
        department: "Science", // Required field by the backend
        topic: "Simulation", // Default topic, could make this configurable
        details: "Created with Simulation Builder",
        prompt: prompt,
        code: code,
      };
      ...
      // Optionally navigate to simulations list or the published simulation
      // navigate('/simulations');
    } catch (error) {
      console.error("Failed to publish simulation:", error);
      setSaveError("Failed to publish simulation. Please try again.");
    } finally {
      setIsSaving(false);
    }
  };
